...
export class CartComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.handler = StripeCheckout.configure({
      key:"pk_test_51Ht31gH7uxtl8hCf10Q2q5933gKtnk7j1kRnXRzsgjm5PTCe4YBhb1PqD3wq0VyNEFa2dKWWbspU3JtXPzVsXPRW00gwjnw2WA",
  ...
  validate() {
    if (!this.quantities.every(data => data > 0)) {
      ...
      this.router.navigate(['/profile/address']).then(() => {
        this.data.warning('You need to enter address.');
      });
    } else {
      this.data.message = '';
      return true;
